...
export class MediaManager extends BaseManager {
   ...
   */
  async uploadMediaImage(params: UploadMediaImageParams): Promise<UploadMediaImageResponse> {
    const response = await ShopeeFetch.fetch<UploadMediaImageResponse>(
      this.config,
      "/media/upload_image",
      {
        method: "POST",
